...
```bash
pnpm install
pnpm run worker              # Starts a Temporal worker (needs a Temporal cluster)
pnpm run trigger-workflows   # Launches the sample workflows through the client
```

Set `TEMPORAL_ADDRESS`, `TEMPORAL_NAMESPACE`, and `TEMPORAL_TASK_QUEUE` in
`.env` to point the worker at your cluster. Use `pnpm run trigger-workflows` to
